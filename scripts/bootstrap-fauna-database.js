...
if (!process.env.GATSBY_FAUNADB_COUNTS_READ) {
  ...
  if (insideNetlify) {
    ...
    console.log('and set a `GATSBY_FAUNADB_COUNTS_READ` value in the "Build environment variables" section')
    process.exit(1)
  }
  ...
  if (!insideNetlify) {
    ...
    ask(chalk.bold('Enter your faunaDB server key'), (err, answer) => {
      ...
      createFaunaDB(process.env.GATSBY_FAUNADB_COUNTS_READ).then(() => {
...
function createFaunaDB(key) {
  ...
  return client.query(q.Create(q.Ref('classes'), { name: 'counts' }))
    .then(() => {
      return client.query(
        q.Create(q.Ref('indexes'), {
          name: 'all_counts',
          source: q.Ref('classes/counts')
        }))
    }).catch((e) => {
